...
bot.on('guildMemberRemove', member => {
  ...
  let gggrole = message.member.roles.filter(r => r.name !=="@everyone" || r.name !== "Dota-key" || r.name !== "EVE-key" || r.name !== "Music-key" || r.name !== "Minecraft-key" || r.name !== "Gmod-key" || r.name !== "CS-key" || r.name !== "SI-key").map(r => r).join(', ')
